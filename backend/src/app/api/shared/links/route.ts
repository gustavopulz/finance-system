...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const iSee = iSeeSnap.docs.map(doc => ({ id: doc.data().userId, name: doc.data().sharedByUser }));
    const seeMeSnap = await firestore.collection('shared_accounts').where('userId', '==', user.id).get();
    const seeMe = seeMeSnap.docs.map(doc => ({ id: doc.data().sharedWithUserId, name: doc.data().sharedWithUserName }));
