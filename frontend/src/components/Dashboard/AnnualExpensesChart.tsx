...
export const AnnualExpensesChart: React.FC<AnnualExpensesChartProps> = ({
  data,
  collaborators,
  loading,
}) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    const monthTotal =
      sorted.reduce((acc, p) => acc + (Number(p.value) || 0), 0) || 1;
  ...
  return (
    <div className="rounded-lg border border-blue-500/60 dark:border-blue-400/60 bg-blue-50/60 dark:bg-blue-500/10 backdrop-blur-sm p-4 h-[28rem] shadow-sm flex flex-col">
      <div className="font-medium mb-2 text-blue-800 dark:text-blue-200">
        Despesas por Mês (Empilhado por Colaborador)
      </div>
      {loading ? (
