...
interface CollaboratorFilterProps {
  ...
  selected: string[]; // collaborator ids
  onChange: (ids: string[]) => void;
}

export function CollaboratorFilter({ collaborators, selected, onChange }: CollaboratorFilterProps) {
  const [open, setOpen] = useState(false);
  const allIds = useMemo(() => collaborators.map(c => c.id), [collaborators]);

  // Sincroniza remoções quando lista muda
  useEffect(() => {
  ...
  function toggleAll() {
    if (allSelected) onChange([]); // limpa tudo
    else onChange(allIds); // seleciona todos
  }
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate max-w-[170px] text-left" title={label}>{label}</span>
      </button>
      {open && (
        <div className="absolute z-30 mt-2 w-72 max-h-80 overflow-auto rounded-lg border border-blue-500/50 dark:border-blue-400/40 bg-blue-50/70 dark:bg-blue-500/10 backdrop-blur-md shadow-lg">
          <div className="sticky top-0 z-10 p-2 border-b border-blue-500/40 dark:border-blue-400/30 bg-blue-100/60 dark:bg-blue-500/20 backdrop-blur flex items-center justify-between text-[10px] font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-200">
            ...
            <button onClick={toggleAll} className="text-blue-600 dark:text-blue-300 hover:underline text-[10px]">
          ...
          <ul className="divide-y divide-blue-500/20 dark:divide-blue-400/10">
            {collaborators.map(c => {
              ...
              return (
                <li key={c.id} className="flex items-center gap-2 px-3 py-2 text-sm cursor-pointer hover:bg-blue-100/40 dark:hover:bg-blue-500/20 transition-colors" onClick={() => toggle(c.id)}>
                  <input
                    type="checkbox"
                    checked={active}
                  ...
                  {active && <span className="text-[10px] text-blue-600 dark:text-blue-300 font-medium">OK</span>}
                </li>
              );
            })}
            {collaborators.length === 0 && (
              <li className="px-3 py-2 text-xs text-blue-600/70 dark:text-blue-300/70">Nenhum colaborador.</li>
            )}
          </ul>
        </div>
      )}
