...
export const SummaryCards: React.FC<SummaryCardsProps> = ({
  ...
  const cards = [
    ...
    {
      label: 'Total Pago',
      value: `${formatCurrency(paid)} (${pct.toFixed(1)}%)`,
      accent: '✅',
    },
  ];
  return (
      ...
      {cards.map((c) => (
        ...
        >
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="text-base opacity-60 select-none">{c.accent}</div>
          </div>
          ...
          <div className="mt-3 h-1.5 w-full rounded bg-blue-200/40 dark:bg-blue-400/20 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-blue-500/70 via-blue-400/70 to-blue-300/60 group-hover:from-blue-600 group-hover:via-blue-500 group-hover:to-blue-400 transition-all"
              style={{ width: '100%' }}
            />
          </div>
        </div>
